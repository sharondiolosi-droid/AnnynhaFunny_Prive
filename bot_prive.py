...
async def modelos(update, context):
    """Comando /modelos - Lista todas as modelos."""
    modelos_lista = get_modelos()
...
async def planos(update, context):
    """Comando /planos - Exibe planos e pre?os."""
    modelos_lista = get_modelos()
    
    if not modelos_lista:
        await update.message.reply_text("?? Nenhuma modelo cadastrada ainda.")
        return
...
async def faq(update, context):
    """Comando /faq - Perguntas frequentes."""
    text = (
...
async def suporte(update, context):
    """Comando /suporte - Contato com suporte."""
    text = (
...
async def add_modelo(update, context):
    ...
    if update.effective_user.id not in ADMIN_CHAT_IDS:
        await update.message.reply_text("? Acesso negado. Voc? n?o ? administrador.")
        return
...
async def drop_all(update, context):
    ...
    await update.message.reply_text(
        ...
        f"Ser? enviado para {len(modelos_lista)} modelo(s).\n\n"
...
async def handle_drop_all(update, context):
    ...
    modelos_lista = drop_info["modelos"]
    canal_tipo = drop_info["canal_tipo"]
    ...
    await update.message.reply_text(
        f"? *DROP ALL ENVIADO!*\n\n"
        f"?? {enviados}/{len(modelos_lista)} canais receberam o conte?do.\n"
        f"?? Canal: {canal_tipo}\n"
        f"? V?lido por {DROP_DURATION_HOURS}h",
        parse_mode=ParseMode.MARKDOWN
    )
...
async def relatorio(update, context):
    ...
    for m in modelos_lista:
        assinantes = get_assinantes(m['id'])
        text += f"?? *{m['nome_completo']}*\n"
        text += f"   ?? @{m['username']}\n"
        text += f"   ?? Free: {m['canal_free']}\n"
        text += f"   ?? FAN: R$ {m['preco_fan']:.2f}\n"
        text += f"   ?? VIP: R$ {m['preco_vip']:.2f}\n"
        text += f"   ?? PRIVE: R$ {m['preco_prive']:.2f}\n"
        text += f"   ?? Assinantes: {len(assinantes)}\n\n"
...
async def main_async():
    ...
    app.add_handler(CommandHandler("listar_modelos", listar_modelos))
    app.add_handler(CommandHandler("admin_modelos", listar_modelos))
